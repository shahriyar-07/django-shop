...
from .models import Product, ProductVisit, ProductGallery
...
class ProductListView(ListView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        context['banners'] = SiteBanner.objects.filter(position__iexact=SiteBanner.SiteBannerPositions.product_list,
                                                       is_active=True)
    ...
    def get_queryset(self):
        ...
        category_url_title = self.kwargs.get('cat')
        ...
        if category_url_title is not None:
            query = query.filter(category__url_title__iexact=category_url_title)
...
class ProductDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context = super().get_context_data()
        context['banners'] = SiteBanner.objects.filter(position__iexact=SiteBanner.SiteBannerPositions.product_detail, is_active=True)
        ...
        context['related_products'] = group_list(list(Product.objects.filter(brand_id=loaded_product.brand_id).exclude(id=loaded_product.id).all()[:12]),3)
