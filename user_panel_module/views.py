...
class MyShopping(ListView):
    ...
    def get_queryset(self):
        ...
        queryset = queryset.filter(user_id=self.request.user.id, is_pay=True)
...
def user_basket(request: HttpRequest):
    current_order, created = Order.objects.prefetch_related('orderdetail_set').get_or_create(is_pay=False,
...
def remove_order_detail(request: HttpRequest):
    ...
    delete_count, deleted_dict = OrderDetail.objects.filter(id=detail_id, order__is_pay=False,
...
def change_order_detail_count(request: HttpRequest):
    ...
    order_detail = OrderDetail.objects.filter(id=detail_id, order__user_id=request.user.id, order__is_pay=False).first()
